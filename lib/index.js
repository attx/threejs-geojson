...
class GeoJsonPreview {
    ...
    createLineStringGeometry(coordinates) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            shapePoints.push(new three_1.Vector2(Math.cos(a) * (this.lineStringHeight * 0.7), Math.sin(a) * this.lineStringWidth));
        }
        const points = coordinates.map((node) => new three_1.Vector3(node[0], node[1], this.lineStringHeight / 2));
        const extrudeSettings = {
            steps: this.lineStringSteps,
            bevelEnabled: false,
            extrudePath: new three_1.CatmullRomCurve3(points),
        };
